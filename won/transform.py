...
def latest_complete(df: pd.DataFrame, min_cols: int = 2) -> pd.DataFrame:
    ...
    non_ind_cols = {"iso3c", "year"}
    if "country" in df.columns:
        non_ind_cols.add("country")

    ind_cols = [c for c in df.columns if c not in non_ind_cols]
...
def correlation_matrix(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cols = [c for c in df.columns if c not in non_ind_cols]
